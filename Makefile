# This Makefile is for the Apache::BabyConnect extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Apache/BabyConnect.pm]
#     AUTHOR => q[root <root@localdomain>]
#     NAME => q[Apache::BabyConnect]
#     PREREQ_PM => {  }
#     VERSION_FROM => q[lib/Apache/BabyConnect.pm]
...
DIRFILESEP = /
DFSEP = $(DIRFILESEP)
NAME = Apache::BabyConnect
NAME_SYM = Apache_BabyConnect
VERSION = 0.92
...
FULLEXT = Apache/BabyConnect
BASEEXT = BabyConnect
PARENT_NAME = Apache
DLBASE = $(BASEEXT)
VERSION_FROM = lib/Apache/BabyConnect.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Apache/BabyConnect.pm
...
TO_INST_PM = lib/Apache/BabyConnect.pm

PM_TO_BLIB = lib/Apache/BabyConnect.pm \
...
DISTNAME = Apache-BabyConnect
DISTVNAME = Apache-BabyConnect-0.92
...
manifypods : pure_all  \
	lib/Apache/BabyConnect.pm \
	lib/Apache/BabyConnect.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Apache/BabyConnect.pm $(INST_MAN3DIR)/Apache::BabyConnect.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:         Apache-BabyConnect' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:      0.92' >> META_new.yml
	$(NOECHO) $(ECHO) 'version_from: lib/Apache/BabyConnect.pm' >> META_new.yml
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,92,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>uses DBI::BabyConnect to initiate persistent database connections</ABSTRACT>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  lib/Apache/BabyConnect.pm blib/lib/Apache/BabyConnect.pm 
